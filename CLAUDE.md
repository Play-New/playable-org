...
- `Org/log.md` — append-only audit. Most-recent on top.
...
- `skills/` — agent workflows + analytical playbooks + design system. Three operational skills (`ingest`, `lint`, `seed`), four analytical playbooks (`ai-exposure`, `value-map`, `reshuffle`, `world-model`), one meta-skill (`new-play`). Cross-cutting docs: `CAPABILITIES.md` (the four-property frame), `STYLE.md` (anti-rhetoric charter), `ROADMAP.md` (the playbook order), `design.py` (single source of truth for the visual language), `_assets/fonts/inter-variable.woff2` (embedded font).
...
- **Structure** — the cited-graph layer in `Org/`, distinct from interpretations in `plays/`. Term borrowed from Simone Cicero on platform design.

Forbidden words in any context: *wiki*, *lens / lente / lenti*, *diagnosis / diagnosi*, *frame*, *view*, *lettura*, *datato/datata/datati/datate*, *substrate / substrato* in user-facing prose (the JSON schema keys `_substrate_id`, `_substrate_evidence`, `_substrate` are kept as internal English schema and are acceptable). Author names of analytical-framework authors (Wardley, Sangeet, Cicero, Burgess, Dorsey, Botha, Karpathy, ...) never appear in any content under `Org/`. They may appear in `skills/` documentation, in `docs/`, in `README.md`, in `CONTRIBUTING.md`, in `CLAUDE.md`.
...
- **No inbox.** Sources arrive via chat upload (one at a time after seed) or via Finder drop into `Org/sources/` (bulk during seed).
...
- 12 mcp tools.
- 3 operational skills: `ingest`, `lint`, `seed`.
...
- `Org/` empty: 3 identity stubs (`# REPLACE ME`), zero other content. Lint Tier 1 reports 3 expected warnings on the stubs (empty `sources` arrays); these go to 0 after seed.
...
The four base playbooks are explicit compositions of established frames: Anthropic Economic Index (ai-exposure), Wardley (value-map), Choudary (reshuffle), Dorsey + Botha (world-model). The term "structure" instead of "substrate" follows Simone Cicero on platform design.
