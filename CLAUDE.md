...
- **Single design system in `skills/design.py`** (currently visual version 4). All viewers compose primitives from there. No bespoke `<style>` blocks beyond a slim playbook-specific extension that uses design tokens. v4 adds editorial chrome helpers (`masthead()`, `colophon()`, `section_head()`, `marginalia()`), 5-stop colour scales per data-viz hue, surface tokens, motion tokens. Backward-compatible with v3.
...
- mcp-server e2e: 252/252 PASS against `mcp-server/test-fixtures/sample-org/`. Pipeline + tool tests (~88) plus design-regression checks for every viewer: 28 on graph (App-pure layout, *Carta sbiadita* palette, inspect-grouping-by-verb, safe-area-inset, Pointer-Events + pinch-zoom, inline favicon, embedded Inter Variable, stripped kinds/edges) and ~25 each on ai-exposure / value-map / reshuffle / world-model (anti-jargon scan, structural shell, framework-specific content, no orphan format placeholders). Plus `design.inline_md` (5 cases), `graph viewer --lang it` (8 cases locking Italian chrome + About modal), `graph build skips README stubs` (3 cases), and `graph autoresearch failure modes` (6 cases locking the five leak families AIRC's first decision-rewriting iteration surfaced: em dash, "Significa X, non Y" formulas, "il grafo dichiara" meta-rhetoric, "dipendenze documentate" Italian graph jargon, and unlinked node references in `node_ids`). Outline & Co. is a fictional creative studio: 5 units, 14 activities, 5 people, 4 stakeholders, 4 commitments, 3 sources; the graph play has 41 nodes / 165 typed relations in the JSON, of which the App-pure viewer renders 32 nodes / 75 edges (corpus and declarative kinds, plus body-markdown link edges, are stripped); the world-model play has 9 capabilities (3 differentiated craft, 6 standard practice), 4 interfaces with today/after states, organization-side and stakeholder-side world-model entries, and 3 missing-capability entries in `pieces_to_build[]` (data layer only, not rendered as a project list on the page).
...
4. Re-render every sample-org artefact so the canonical HTML output stays in sync.
