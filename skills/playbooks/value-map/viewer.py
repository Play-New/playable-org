...
.modal .stage-pill { display: inline-block; padding: 2px 10px; border-radius: 999px; font-size: 0.7rem; font-weight: 500; text-transform: uppercase; letter-spacing: 0.04em; border: 1px solid var(--line); }
.modal .stage-pill.genesis { background: var(--success-bg); color: var(--success); border-color: #a7f3d0; }
.modal .stage-pill.custom { background: var(--info-bg); color: var(--info); border-color: #bfdbfe; }
.modal .stage-pill.product { background: var(--info-bg); color: var(--info); border-color: #bfdbfe; }
.modal .stage-pill.commodity { background: var(--bg-soft); color: var(--fg); border-color: var(--line); }
...
document.addEventListener('click', (e) => {{
  const node = e.target.closest('.node, .comp-card');
  if (!node) return;
