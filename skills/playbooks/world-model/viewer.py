...
/* Intro: editorial prose, no card frame. */
.intro { margin: 0 0 56px; }
.intro h2 { font-family: var(--font-display); font-size: 1.5rem; font-weight: 500; letter-spacing: -0.02em; margin: 0 0 16px; }
.intro p { font-size: 0.95rem; line-height: 1.7; margin: 0 0 14px; color: var(--fg); max-width: 720px; }
.intro p strong { font-weight: 500; }
.intro .pull { padding: 14px 0 14px 18px; margin: 22px 0; font-size: 1.0rem; color: var(--fg); border-left: 2px solid var(--fg); max-width: 720px; line-height: 1.65; }
...
/* Shared principle — editorial pull, no filled background. */
.principle-block { padding: 18px 0 18px 18px; margin: 36px 0; border-left: 2px solid var(--moat); max-width: 720px; }
...
<body>
  <div class="container">
    ...
    <div class="frame-diagram">
      {frame_diagram_svg}
      <div class="frame-diagram-caption">The five layers, schematic. The page below is this same shape filled with the organization's actual data — people, touchpoints, compositions, knowledge, capabilities — plus the failure-signals that name where a needed function isn't there yet.</div>
    </div>
    ...
    <div class="stack">
      ...
      <div class="stack-layer layer-stakeholders">
        <div class="layer-head">
          <div class="layer-name">People</div>
      ...
      <div class="stack-layer layer-interfaces">
        <div class="layer-head">
          <div class="layer-name">Touchpoints</div>
      ...
      <div class="stack-layer layer-intelligence">
        <div class="layer-head">
          <div class="layer-name">Coordination</div>
      ...
      <div class="stack-layer layer-worldmodel">
        <div class="layer-head">
          <div class="layer-name">Knowledge</div>
...
function renderStakeholderPopover(stype) {{
  ...
  let html = `<div class="eyebrow">people · type</div>`;
...
function renderInterfacePopover(idx) {{
  ...
  if (!ifc) return '';
  let html = `<div class="eyebrow">touchpoint</div>`;
...
function renderSignalPopover(idx) {{
  const s = SIGNALS[idx];
  if (!s) return '';
  let html = `<div class="eyebrow">where the structure is thin</div>`;
...
def render_html(d: dict, title: str) -> str:
  ...
  <text x="360" y="70" text-anchor="middle" font-size="14" fill="#FFFFFF" font-weight="600">People</text>
  ...
  <text x="360" y="152" text-anchor="middle" font-size="14" fill="#1a1a1a" font-weight="600">Touchpoints</text>
  ...
  <text x="360" y="234" text-anchor="middle" font-size="14" fill="#1a1a1a" font-weight="600">Coordination</text>
  ...
  <text x="360" y="316" text-anchor="middle" font-size="14" fill="#1a1a1a" font-weight="600">Knowledge</text>
